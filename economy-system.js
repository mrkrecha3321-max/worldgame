    ...
    processMonthly(state, turnNumber) {
      ...
      for (const [countryId, country] of Object.entries(state.countries)) {
        try {
          ...
          const baseInflationDrift = 2.2;
          const interestDampening = (cb.baseRate - 2.5) * 0.35;
          const outputGap = (netAnnualGrowthRate - basePotentialGrowth) * 0.20;
          const newInflation = eco.inflation + (baseInflationDrift + outputGap - interestDampening - eco.inflation) * 0.12 + R.gaussian(0, 0.06);
          eco.inflation = V.sanitizeNumber(newInflation, 2.5, -1.0, 85.0);
          ...
          if (eco.gdpHistory.length > 120) eco.gdpHistory.shift();
          eco.inflationHistory.push(eco.inflation);
          if (eco.inflationHistory.length > 120) eco.inflationHistory.shift();
          eco.unemploymentHistory.push(eco.unemployment);
          if (eco.unemploymentHistory.length > 120) eco.unemploymentHistory.shift();
