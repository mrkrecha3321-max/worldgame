...
(function () {
  ...
  const WorldMapUI = {
    ...
    initD3Map(container, geoData, state, playerCountry) {
      ...
      // 9. D3 Zoom & Pan Setup
      // (feature-detection: w środowiskach bez pełnej implementacji SVG — np. jsdom —
      // d3-zoom opiera się na transform.baseVal, które tam nie istnieje; mapa
      // pozostaje wtedy statyczna zamiast rzucać wyjątki przy każdej interakcji)
      const svgSupportsZoom = typeof svgEl.getScreenCTM === 'function' && svgEl.transform !== undefined;
      if (svgSupportsZoom) {
