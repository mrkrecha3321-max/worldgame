...
(function () {
  ...
  const ModalUI = {
    ...
    /**
     * Standardowy komunikat o błędzie operacji (zamiast window.alert).
     * @param {string} title
     * @param {string} reason - opis przyczyny niepowodzenia
     */
      ...
      this.show({
        ...
        contentHtml: `
          <div style="padding: 4px 0;">
            <p style="color: var(--text-secondary); line-height: 1.5; font-size: 12px; margin: 0;">${reason || 'Operacja nie powiodła się z nieznanych przyczyn.'}</p>
          </div>
        `,
        buttons: [
          { text: 'Rozumiem', class: 'wf-btn-primary', onClick: () => this.close() }
        ]
      });
    },
