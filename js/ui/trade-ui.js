...
(function () {
  ...
  const TradeUI = {
              ...
              <div class="grid-2">
                ...
                <div class="slider-group">
                  ...
                  <div style="font-size: 9.5px; color: var(--text-muted); margin-top: 2px;">
                    AI zaakceptuje cenę do ~115% ceny spot danego surowca.
                  </div>
                </div>
              </div>
    ...
    bindEvents(container) {
      ...
      if (proposeBtn) {
        proposeBtn.onclick = () => {
          ...
          if (!result.success) {
            window.WorldForge.UI.Modal.showError('Oferta Odrzucona', result.reason);
          }
    ...
    showTradeOrderModal(country, resId, action, container) {
        ...
        <div style="display: flex; flex-direction: column; gap: 10px;">
          ...
          <div class="slider-group">
            <div style="display: flex; justify-content: space-between; align-items: baseline;">
              <label style="font-size: 11px; font-weight: 600;">Szacowana cena wykonania ($/jedn.):</label>
              <span style="font-size: 10px; color: var(--text-muted);">wraz z wpływem na rynek</span>
            </div>
            <div style="display: flex; gap: 8px; align-items: center; background: var(--bg-panel-secondary); border: 1px solid var(--border); border-radius: var(--border-radius-xs); padding: 7px 10px;">
              <strong class="font-mono ${isBuy ? 'text-negative' : 'text-positive'}" id="trade-order-exec-price" style="font-size: 14px;">$${spotPrice.toFixed(1)}</strong>
              <span id="trade-order-impact" style="font-size: 10px; color: var(--text-muted);">wpływ na rynek: 0.0%</span>
            </div>
            <div style="font-size: 10px; color: var(--text-muted); margin-top: 3px;">
              Zlecenia giełdowe wykonują się po cenie rynkowej. Duże wolumeny podnoszą koszt zakupu i obniżają cenę sprzedaży.
              Niestandardowe ceny negocjuj w umowach bilateralnych poniżej.
            </div>
          </div>
        ...
        buttons: [
          { text: 'Anuluj', class: 'wf-btn-secondary', autoClose: true },
          {
            ...
            onClick: () => {
              ...
              if (!res.success) {
                window.WorldForge.UI.Modal.showError('Zlecenie Odrzucone', res.reason);
              }
      ...
      // Recalculate total value live (z wpływem zlecenia na rynek)
      ...
      const execPriceEl = document.getElementById('trade-order-exec-price');
      const impactEl = document.getElementById('trade-order-impact');
      const TradeSys = window.WorldForge.Systems.Trade;

      const updateOrderTotal = () => {
        const amt = parseFloat(amountInput?.value || 0) || 0;
        const impact = TradeSys.computeMarketImpact(amt, resId, state);
        const execPrice = spotPrice * (isBuy ? (1 + impact) : (1 - impact));
        const total = Math.round(amt * execPrice * 1000);
        if (totalValEl) totalValEl.innerHTML = F.money(total, 'USD');
        if (execPriceEl) execPriceEl.textContent = '$' + execPrice.toFixed(1);
        if (impactEl) {
          impactEl.textContent = `wpływ na rynek: ${(isBuy ? '+' : '-')}${(impact * 100).toFixed(1)}%`;
          impactEl.style.color = impact > 0.15 ? 'var(--negative)' : 'var(--text-muted)';
        }
      };
